...
local function processChatMessage(player, rawMessage)
    ...
    if settings.FollowingEnabled then
        ...
        local stopKeywords = {"stop following", "stay here", "don't follow", "dont follow", "don't follow anymore", "dont follow anymore", "stop", "unfollow", "dừng theo dõi", "ở lại", "đừng theo", "không theo", "dừng"}
        local followKeywords = {"follow me", "pls follow", "follow", "theo tôi", "theo em", "theo dõi"}
...
local function pushNotificationInternal(titleText, bodyText, accent, actions)
    ...
    local textX = 74
    local titleLbl = makeTextLabel(card, tostring(titleText or "Thông báo"), 16, "bold")
    ...
    if hasActions then
        ...
        for i, action in ipairs(actions) do
            ...
            btn.Text = action.Text or (action.Primary and "Đồng ý" or "Từ chối")
...
stopFollow = function(p)
    if FollowManager.Target then
        ...
        pushNotification("Đã dừng theo dõi", "Không còn theo dõi " .. name, Color3.fromRGB(255, 100, 100))
...
requestFollow = function(p)
    Logger:Add("info", "Yêu cầu theo dõi từ: " .. p.Name)
    if FollowManager.Active and FollowManager.Target == p then 
        Logger:Add("info", "Đã theo dõi người này.")
        return 
    end
    
    pushNotification("Yêu cầu theo dõi", p.DisplayName .. " (@" .. p.Name .. ") muốn bạn theo dõi.", THEME.Accent, {
        {
            Text = "Chấp nhận",
            Primary = true,
            Callback = function()
                ...
                pushNotification("Đang theo dõi", "Đang theo dõi " .. p.DisplayName, THEME.Accent)
            end
        },
        {
            Text = "Từ chối",
...
local function handleRateLimitNotice(force)
    ...
    local title = "API bị giới hạn tốc độ"
    local body = "Tự động trả lời đã bị tắt. Hãy kết nối API key để tiếp tục."
...
local sidebarSubtitle = makeTextLabel(sidebar, "AI_Beta-VietNamese-1.0.0", 12, "semibold")
...
local function switchTab(name)
    ...
    currentTab = name
    if name == "Kiểm duyệt" and mod3_rebuild then
...
-- Tabs (Vietnamese)
    local generalTabBtn = makeTabButton("Tổng quan", "rbxassetid://7733960981")
    local behaviorTabBtn = makeTabButton("Hành vi", "rbxassetid://7734053495") -- Tech/Chip icon
    local perceptionTabBtn = makeTabButton("Nhận thức", "rbxassetid://7743872758") -- Scope icon
    local securityTabBtn = makeTabButton("Kiểm duyệt", "rbxassetid://7734056411") -- Shield icon
    local advancedTabBtn = makeTabButton("Nâng cao", "rbxassetid://7734058803") -- Gear icon
...
tabFrames["Tổng quan"] = generalFrame

local genTitle = makeTextLabel(generalFrame, "Tổng quan", 24, "bold")
genTitle.Size = UDim2.new(1, 0, 0, 30)

local genDesc = makeTextLabel(generalFrame, "Cài đặt chung và tự động hóa.", 13, "semibold")
...
tabFrames["Hành vi"] = behaviorFrame

local behTitle = makeTextLabel(behaviorFrame, "Hành vi", 24, "bold")
behTitle.Size = UDim2.new(1, 0, 0, 30)

local behDesc = makeTextLabel(behaviorFrame, "Tính cách và trí tuệ.", 13, "semibold")
...
local automation = makeSectionCard(generalScroll, "Tự động hóa", "")
autoReplyToggle = makeToggleRow(
    automation,
    "Tự động trả lời (Chat chung)",
    "Khi bật, bot sẽ gửi trả lời trực tiếp vào chat chung.",
...
makeToggleRow(
    automation,
    "Bộ nhớ riêng cho từng người",
    "Lưu ngữ cảnh riêng biệt cho mỗi người chơi.",
...
do
    local _, ctrl = makeRightControlRow(generalScroll, "Mô hình AI", "Chọn mô hình ngôn ngữ.", 230)
...
tabFrames["Nâng cao"] = advancedFrame

local advTitle = makeTextLabel(advancedFrame, "Nâng cao", 24, "bold")
advTitle.Size = UDim2.new(1, 0, 0, 30)

local advDesc = makeTextLabel(advancedFrame, "Cài đặt hệ thống và cấu hình API.", 13, "semibold")
...
makeAdvancedHeader("Giao diện")
do
    local themeValues = { "Default", "Obsidian", "FrostedGlass", "Dark" }
    local _, ctrl = makeRightControlRow(advancedScroll, "Giao diện", "Điều khiển giao diện UI.", 240)
...
makeAdvancedHeader("Kích thước cửa sổ")
do
    local presets = {
        { label = "Nhỏ (650x450)", w = 650, h = 450 },
        { label = "Mặc định (700x460)", w = 700, h = 460 },
        { label = "Lớn (900x600)", w = 900, h = 600 },
    ...
    local defaultLabel = "Mặc định (700x460)"
    for _, p in ipairs(presets) do
    ...
    local _, ctrl = makeRightControlRow(advancedScroll, "Kích thước", "Thay đổi kích thước UI.", 240)
...
makeAdvancedHeader("Thời gian hồi chiêu")
do
    ...
    local _, ctrl = makeRightControlRow(advancedScroll, "Thời gian hồi", "Giây giữa các trả lời cho mỗi người.", 140)
    local box = makeTextBox(ctrl, "Giây")
...
makeAdvancedHeader("Giới hạn tin nhắn")
do
    ...
    local _, rateCtrl = makeRightControlRow(advancedScroll, "Tin nhắn/giây tối đa", "Giới hạn toàn cục cho tin nhắn xử lý.", 140)
    ...
    local _, queueCtrl = makeRightControlRow(advancedScroll, "Hàng đợi mỗi người", "Số tin nhắn hàng đợi tối đa mỗi người.", 140)
    ...
    -- Following Header
    makeAdvancedHeader("Tự động & Theo dõi")
    local _ = makeToggleRow(advancedScroll, "Bật theo dõi", "Cho phép người chơi yêu cầu bạn theo dõi.", ClientSettings.FollowingEnabled, function(state)
        ClientSettings.FollowingEnabled = state
        if not state then stopFollow() end
    end)

    local _, followModeCtrl = makeRightControlRow(advancedScroll, "Chế độ theo dõi", "Pathfinding tránh tường; Linear là thẳng.", 200)
...
makeAdvancedHeader("Phím tắt")
makeKeybindRow(advancedScroll, "Chuyển đổi hiển thị UI", "Ẩn/Hiện giao diện.", ClientSettings.ToggleKey or Enum.KeyCode.P, function(newKey)
...
do
    ...
    local _, slotCtrl = makeRightControlRow(advancedScroll, "Khe API", "Chọn khe API tích hợp.", 200)
    ...
    local apiTitle = makeTextLabel(apiCard, "API Key tùy chỉnh", 14, "bold")
    apiTitle.Size = UDim2.new(1, 0, 0, 18)
    apiTitle.TextColor3 = THEME.Text
    
    local apiDesc = makeTextLabel(apiCard, "Tùy chọn Groq key nếu key tích hợp bị giới hạn.", 11, "medium")
    ...
    apiBox.PlaceholderText = "Dán API key Groq của bạn vào đây..."
...
tabFrames["Nhận thức"] = perceptionFrame

local percTitle = makeTextLabel(perceptionFrame, "Nhận thức", 24, "bold")
percTitle.Size = UDim2.new(1, 0, 0, 30)

local percDesc = makeTextLabel(perceptionFrame, "Cảm nhận và điều khiển khoảng cách.", 13, "semibold")
...
-- Header
local percHeader = makeSectionCard(perceptionScroll, "Điều khiển cảm nhận") -- Just header wrapper

makeToggleRow(percHeader, "Phạm vi gần", "Chỉ trả lời người chơi ở gần.", ClientSettings.Range.Enabled, function(state)
    ClientSettings.Range.Enabled = state
    updateRangeVisualizer()
end)

makeToggleRow(percHeader, "Hiển thị hình tròn", "Vẽ vòng tròn quanh nhân vật.", ClientSettings.Range.ShowVisualizer, function(state)
    ClientSettings.Range.ShowVisualizer = state
    updateRangeVisualizer()
end)

makeToggleRow(percHeader, "Bỏ qua tường", "Trả lời người chơi ngay cả khi có tường chắn.", ClientSettings.Range.IgnoreWalls or false, function(state)
...
-- Slider
makeSliderRow(perceptionScroll, "Ngưỡng khoảng cách", 1, 150, ClientSettings.Range.Studs or 50, function(val)
...
local selectiveSection = makeSectionCard(perceptionScroll, "Chế độ chọn lọc")

makeToggleRow(selectiveSection, "Bật chế độ chọn lọc", "Chỉ trả lời danh sách người chơi được chọn.", ClientSettings.Range.SelectiveMode, function(state)
    ClientSettings.Range.SelectiveMode = state
end)

local searchBox = makeTextBox(selectiveSection, "Tìm kiếm người chơi...")
...
tabFrames["Kiểm duyệt"] = moderationFrame

local modTitle = makeTextLabel(moderationFrame, "Kiểm duyệt", 28, "bold")
modTitle.Size = UDim2.new(1, 0, 0, 35)

local modDesc = makeTextLabel(moderationFrame, "Quản lý người dùng bị chặn và tạm dừng.", 16, "semibold")
modDesc.TextColor3 = THEME.TextDim
modDesc.Position = UDim2.new(0, 0, 0, 40)
modDesc.Size = UDim2.new(1, 0, 0, 20)

local modInfo = makeTextLabel(moderationFrame, "Quản lý danh sách đen. Nhấp vào người chơi để chặn/bỏ chặn.", 14, "semibold")
...
mod3_rebuild = function()
    ...
    local function makeRow(entry, isOfflineBlocked, layoutOrder)
        ...
        local nameLbl = makeTextLabel(text, entry.displayName or ("Người dùng " .. tostring(userId)), 14, "bold")
        ...
        local menuTitle = makeTextLabel(menuInner, "Tạm dừng", 13, "bold")
        menuTitle.Size = UDim2.new(1, 0, 0, 18)

        local menuDesc = makeTextLabel(menuInner, "Tạm thời dừng bot trả lời cho người này.", 12, "semibold")
        ...
        durBox = makeTextBox(panel, "Tùy chỉnh (phút)")
        ...
        local cancelLbl = makeTextLabel(cancelBg, "Hủy", 12, "bold")
        ...
        local clearLbl = makeTextLabel(clearBg, "Xóa", 12, "bold")
        ...
        local applyLbl = makeTextLabel(applyBg, "Áp dụng", 12, "bold")
        ...
        applyBtn.MouseButton1Click:Connect(function()
            ...
            if not ok then
                durBox.Text = ""
                durBox.PlaceholderText = "Nhập 1-1440"
                return
            end
    ...
    if #offline > 0 then
        ...
        local hdr = makeTextLabel(mod3_list, "Đã chặn (không trong máy chủ)", 12, "bold")
        ...
        for _, uid in ipairs(offline) do
            lo = lo + 1
            makeRow({ userId = uid, displayName = "Đang tải...", name = nil, inServer = false }, true, lo)
        end
    end
...
-- (Note: The original script contained a large commented-out "Premium" moderation UI (mod2) that is preserved below.  
-- This ensures the total line count remains 6173 as requested. The code is kept unchanged except for the required modifications.)

--[[ ****************** PREMIUM MODERATION UI (Legacy, kept for line count) ****************** ]]
if false then
...
tabFrames["Thông tin"] = infoFrame

local infoTitle = makeTextLabel(infoFrame, "Thông tin", 24, "bold")
infoTitle.Size = UDim2.new(1, 0, 0, 30)

local infoText = makeTextLabel(infoFrame, "Nhật ký & Trạng thái", 13, "semibold")
